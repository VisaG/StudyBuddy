...
namespace StudyBuddy.Views
{
	public partial class GroupDetailPage : ContentPage
	{
		...
		public GroupDetailPage(int GroupId)
		{
			...
			Group grp = DataUtil.getGroupById(GroupId);
			...
			for (int j = 0; j < studlist.Count; j++)
			{
				...
				Image img = new Image
				{
					Source =
					 Device.OnPlatform(
				iOS: ImageSource.FromFile(studlist[j].fileImage),
				Android: ImageSource.FromFile(studlist[j].fileImage),
				WinPhone: ImageSource.FromFile(studlist[j].fileImage))
