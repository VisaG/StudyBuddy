		...
		public LoginPageViewModel(EasyMobileServiceClient client) : base(client)
		{
			...
			NavigationCommand = new Command(async() =>
		   {
			   ...
			   if (items.Count > 0)
			   {
				   ...
				   //Settings.Default.Students = await studentTable.ToCollectionAsync();
				 //  Settings.Default.GroupStudents = await studentTable.ToCollectionAsync();
				 //  Settings.Default.InterestStudents = await studentTable.ToCollectionAsync();
					//Settings.Default.Groups = await studentTable.ToCollectionAsync();
				 //  Settings.Default.Interests = await studentTable.ToCollectionAsync();
					//Settings.Default.Requests = await studentTable.ToCollectionAsync();
